...
const variants = {
  ...
  closed: { opacity: 0, y: 0, transition: { duration: 0.35 } }
};
        ...
        <div className="d-flex flex-column vw-100 h-100">
          ...
          <motion.div initial="closed" animate="open" variants={variants}>
            <Navbar.Collapse id="basic-navbar-nav">
              ...
              <Nav
                className="me-auto overflow-auto border-bottom border-top"
                variant="tabs"
                style={{ height: getHeight() }}
              >
                <Nav.Link
                  className={`${styles.navbarLinks} d-flex justify-content-between border-bottom m-0 rounded-0 text-dark fs-5`}
                  onClick={() => onClick(0)}
                >
                  GROUP STUDIES
                  {' '}
                ...
                <Link
                  href="#"
                  className={`${styles.navbarLinks} border-bottom m-0 rounded-0 text-dark fs-5 nav-link`}
                >
                  ABOUT US
                </Link>
                <Link
              ...
              <Nav className="mt-3 mb-3">
                {!isLoggedIn && (
                  <Link
                    href="/signin"
                    className="btn btn-outline-dark text-nowrap mx-3"
                  >
                    SIGN IN
                  </Link>
                )}
                <ButtonJoinNow className="text-nowrap mx-3 mt-3" />
              </Nav>
            </Navbar.Collapse>
          </motion.div>
        </div>
